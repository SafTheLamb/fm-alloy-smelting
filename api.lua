...
if not AlloySmelting then
	...
	function AlloySmelting.smelt_in_kiln(recipe_name, catalyst_ingredient, recipe_scale, recipe_category)
		...
		frep.set_categories(recipe, {recipe_category or "kiln-smelting"})
	...
	function AlloySmelting.add_catalyst(recipe_name, catalyst_ingredient, recipe_scale)
		...
		if recipe_scale then
			...
			for _,result in pairs(recipe.results) do
				if result.independent_probability and recipe_scale * result.independent_probability < 1 then
					result.independent_probability = recipe_scale * result.independent_probability
				else
